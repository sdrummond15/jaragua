...
<script type="text/javascript">
    ...
    window.onload = function(){
        ...
        id('user_phone').onkeyup = function(){
			...
			<table class="acymailing_form">
				...
				<tr>
					...
					<?php foreach($fieldsToDisplay as $oneField){
						...
						}elseif($oneField == 'phone' AND empty($extraFields[$oneField])){

							if($displayOutside) echo '<td><label for="user_phone_'.$formName.'">'.$phoneCaption.'</label></td>'; ?>

							<td class="acyfield_<?php echo $oneField; ?>">

								<input id="user_phone" class="inputbox user_phone" type="text" name="user[phone]" placeholder="Telefone" style="width:<?php echo $fieldsize; ?>" maxlength="15"/>
