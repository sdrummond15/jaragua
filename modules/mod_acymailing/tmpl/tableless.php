			...
			<div class="acymailing_form">
					<?php foreach($fieldsToDisplay as $oneField){
						...
						if($oneField == 'name' AND empty($extraFields[$oneField])){
							...
							<span class="acyfield_<?php echo $oneField; ?>"><input id="user_email_<?php echo $formName; ?>" <?php if(!empty($identifiedUser->userid)) echo 'disabled="disabled" '; ?> placeholder="<?php echo $emailCaption;?>" class="inputbox" type="text" name="user[email]" style="width:<?php echo $fieldsize; ?>" value="<?php if(!empty($identifiedUser->userid)) echo $identifiedUser->email; ?>" /></span>
							<?php
