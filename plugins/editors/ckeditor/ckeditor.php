...
class plgEditorCKeditor extends JPlugin
{
	...
	function onDisplay( $name, $content, $width, $height, $col, $row, $buttons = true, $id = null, $asset = null, $author = null)
	{
		...
		//check if extra plugins are defined in Custom JavaScript field
...
";
	...
	function _displayButtons($name, $buttons, $asset, $author)
	{
		...
		if (is_array($buttons) || (is_bool($buttons) && $buttons)) {
			 ...
			 */
			$return .= "\n<div id=\"editor-xtd-buttons-".$name."\" class=\"btn-toolbar\">\n";
			...
			foreach ($results as $button)
			{
				...
				if ( $button->get('name') ) {
					...
					$return .= "<div class=\"btn-wrapper\"><button class=\"btn btn-small ".$button->get('name')."\"><a ".$modal." title=\"".$button->get('text')."\" ".$href." ".$onclick." rel=\"".$button->get('options')."\">".$button->get('text')."</a></button></div>\n";
				}
			}
