...
class JemOutput
{
	...
	static public function formatTimeOtimize($startTime = '', $endTime = '', $format = '')
	{
		if(!empty($startTime) && !empty($endTime)){
			return 'de ' . self::formattime( $startTime, $format, false ) . ' às ' . self::formattime( $endTime, $format, false ) .'h';
		}else{
            return 'às ' . self::formattime( $startTime, $format, false ) .'h';
		}
	}
