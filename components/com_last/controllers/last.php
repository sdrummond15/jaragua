...
class LastControllerLast extends JControllerForm
{
    public function insert()
    {
        ...
        try {
            ...
            if (!isset($_POST['id']) || empty($_POST['id'])) {
                ...
                $mainframe->redirect(JRoute::_('index.php?option=com_last&view=inscrever'));
                return false;
            }
            //Nome
            ...
            $query
                ->insert($db->quoteName('#__doing_subscribe'))
        ...
        $mainframe->redirect(JRoute::_('index.php?option=com_last&view=inscrever&id=' . $id));
...
class PHP_email extends PHPMailer
{
    ...
    public static function email_confirma_contrato($dataForm)
    {
        ...
        $mail->setBody(
            '<html>'
            . '<body>'
            . '<p>Inscrito no jornal: ' . $doing[0]->name . ' com sucesso.</p>'
        ...
        $mail->setBody(
            '<html>'
            . '<body>'
            . '<p>Inscrição no jornal: ' . $doing[0]->name . ' com sucesso.</p>'
