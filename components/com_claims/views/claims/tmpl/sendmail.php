...
$msg_partner = $_POST['msg_partner'];

$msg_partner = str_replace('<p>__</p>', '', $msg_partner);
?>
...
try {
    ...
    if(!empty($subject)){
        ...
        $subject = 'Sugestão/Reclamação do Associado Jaraguá';
    ...
    //E-mail para sócio
    $mailsocio = JFactory::getMailer();
    $mailsocio->addRecipient($email);
    $mailsocio->setSender(array($mailfrom, $fromname));
    $mailsocio->isHtml();
    $mailsocio->setSubject('Ouvidoria Jaraguá Country Club');
    $mailsocio->setBody($msg_partner);
    $sentsocio = $mailsocio->Send();
    ...
    if(!empty($sucesso)){
        ...
        $sucesso = 'Obrigado pela<br>sua sugestão ou reclamação!';
    ...
    if(!empty($erro)){
        ...
        $erro = 'ERRO Obrigado pela<br>sua sugestão ou reclamação!';
